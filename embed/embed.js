...
const createContribution = async ({ data }) => {
  ...
  const contributions = await Promise.all(
    users.map((user) => requestContribution(user))
  );

  const results = contributions.map((c) =>
    c.data.contributions.find(findToday)
  );
  ...
  embed.setDescription(
    "매일 6시 정각에 커밋 상태를 확인하고 알림을 전송합니다."
  );
...
const createHelpEmbed = () => {
  ...
  embed.addFields({
    name: "!noti",
    value: "그룹 내 유저들에게 공지 메세지를 전송합니다.",
    inline: true,
  });
  embed.addFields({ ...space });
